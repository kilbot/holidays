  ...
  for (const placement of input.placements) {
    ...
    if (placement.overlaps.length > 0) {
      ...
      out.push({
        ...
        detail: `There was no free run of ${placement.days} days its Lock allows, so it is placed on top of ${names}. Those Days are priced once, at ${capsule.name}'s rates. Drag something, or drop a Capsule.`,
        dates: datesOf(placement),
        capsuleId: capsule.id,
      });
    }
